...
export default function SettingsClient({ profile, allProfiles }: Props) {
  ...
  const [result, setResult] = useState<{ message: string; action: string; credentials: Credentials; email_sent?: boolean } | null>(null);
      ...
      {isAdmin && (
        <div className="card p-5">
          <div className="flex items-center justify-between border-b border-[#2d3142] pb-3 mb-4">
            ...
            {!showAdd && (
              <button onClick={() => setShowAdd(true)} className="btn-primary text-xs py-1.5 px-3 flex items-center gap-1.5">
              ...
              {result ? (
                <div className="space-y-4">
                  <div className={cn(
                    ...
                    <p className="text-xs text-slate-400 mb-2">{result.message}</p>
                    {result.email_sent && (
                      <div className="flex items-center gap-1.5 mb-4">
                        ...
                        <p className="text-xs text-sky-400">Invite email sent with login credentials</p>
                      </div>
                    )}
                    {!result.email_sent && (
                      <p className="text-xs text-yellow-500/80 mb-4">📋 No invite email sent — share credentials manually below</p>
                    )}
                  ...
                  <div className="flex gap-2">
                    <button
                      onClick={() => { setResult(null); setAddEmail(''); setAddName(''); setAddPassword(generatePassword()); setSendInviteEmail(true); }}
                ...
                <form onSubmit={handleAddMember} className="space-y-4 p-4 bg-[#1a1d27] border border-sky-500/20 rounded-xl">
                  <div className="flex items-center justify-between mb-1">
                    <div>
                      <h3 className="text-sm font-semibold text-sky-400">Invite Team Member</h3>
                      <p className="text-xs text-slate-500 mt-0.5">Create their account and optionally send them an invite email</p>
                    </div>
                  ...
                  <label className="flex items-center gap-3 p-3 bg-[#0f1117] border border-[#2d3142] rounded-lg cursor-pointer hover:border-sky-500/30 transition-colors">
                    <div className="relative flex-shrink-0">
                      <input
                        ...
                        className="sr-only"
                      />
                      <div className={`w-9 h-5 rounded-full transition-colors ${sendInviteEmail ? 'bg-sky-500' : 'bg-slate-600'}`}>
                        <div className={`absolute top-0.5 left-0.5 w-4 h-4 bg-white rounded-full shadow transition-transform ${sendInviteEmail ? 'translate-x-4' : 'translate-x-0'}`} />
                      </div>
                    </div>
                    <div>
                      <p className="text-xs font-medium text-slate-200">Send invite email</p>
                      <p className="text-[11px] text-slate-500">Email login credentials to the new member automatically</p>
                    </div>
                  ...
                  <button type="submit" disabled={addLoading || !addEmail.trim()} className="btn-primary w-full justify-center py-2.5">
                    {addLoading ? (
                      <span className="flex items-center gap-2">
                        ...
                        {sendInviteEmail ? 'Creating & Sending Invite…' : 'Creating Account…'}
                      </span>
                    ) : sendInviteEmail ? '✉️ Create Account & Send Invite Email' : 'Create Account & Get Credentials'}
                  </button>
                </form>
              )}
            </div>
