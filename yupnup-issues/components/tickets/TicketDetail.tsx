...
export default function TicketDetail({ ticket, members, currentProfile }: Props) {
  ...
  // Everyone can do everything except delete (admin only)
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-4 animate-fade-in">
      ...
      <div className="flex items-center gap-2 text-sm text-slate-500">
        <Link href="/dashboard/tickets" className="hover:text-slate-300 transition-colors">← All Tickets</Link>
        <span>/</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          <div className="card p-5">
            <div className="flex items-start justify-between gap-4 mb-3">
              ...
              {/* Edit + Delete — visible to all, delete admin only */}
              <div className="flex gap-2 flex-shrink-0">
                <Link href={`/dashboard/tickets/${ticket.id}/edit`} className="btn-secondary text-xs py-1.5 px-3">
                  ✏️ Edit
                </Link>
                {canDelete && (
              ...
              <div className="flex flex-wrap gap-3">
                {ticket.screenshots.map((ss: any) => (
                  <button key={ss.id} onClick={() => setSelectedImage(ss.url)} className="group relative">
                    <Image src={ss.url} alt={ss.filename} width={120} height={90}
          ...
          {/* ── Status Change — ALL users can change ── */}
          <div className="card p-5">
            ...
            <div className="flex flex-wrap gap-2">
              {STATUS_FLOW.map((s) => (
                <button
                  ...
                  className={cn(
                    'badge cursor-pointer transition-all hover:scale-105 active:scale-95',
                    s === ticket.status
                      ? `${STATUS_CONFIG[s].bg} ${STATUS_CONFIG[s].color} ring-1 ring-current font-bold`
                      : 'bg-[#252836] border-[#2d3142] text-slate-400 hover:text-white hover:border-slate-500 disabled:cursor-default'
                  )}
                >
          ...
          <div className="card p-5">
            ...
            <div className="space-y-4 mb-5">
              {(!ticket.comments || ticket.comments.length === 0) && (
                <p className="text-sm text-slate-500 text-center py-4">No comments yet. Add one below.</p>
              )}
              {ticket.comments?.map((c: any) => (
                <div key={c.id} className="flex gap-3">
                  <div className="w-8 h-8 rounded-full bg-gradient-to-br from-sky-500 to-indigo-600 flex items-center justify-center text-xs font-bold text-white flex-shrink-0 mt-0.5">
                    {c.author?.full_name ? getInitials(c.author.full_name) : c.author?.email?.[0]?.toUpperCase() || '?'}
                  </div>
                  <div className="flex-1">
            ...
            <form onSubmit={handleAddComment} className="flex gap-3">
              <div className="w-8 h-8 rounded-full bg-gradient-to-br from-sky-500 to-indigo-600 flex items-center justify-center text-xs font-bold text-white flex-shrink-0 mt-1">
                {currentProfile?.full_name ? getInitials(currentProfile.full_name) : currentProfile?.email?.[0]?.toUpperCase() || '?'}
              </div>
              <div className="flex-1 space-y-2">
                <textarea
                  value={comment} onChange={e => setComment(e.target.value)}
                  className="textarea" rows={3} placeholder="Write a comment…"
                />
                    ...
                    <div>
                      <p className="text-xs text-slate-400">
                        ...
                        {log.old_value && log.new_value && (
                          <> · <span className="line-through text-slate-600">{log.old_value}</span> → <span className="text-sky-400">{log.new_value}</span></>
                        )}
                      </p>
        ...
        <div className="space-y-4">
          <div className="card p-4 space-y-4">
            <h3 className="text-xs font-bold text-slate-500 uppercase tracking-widest">Ticket Info</h3>

            <div>
              <p className="text-xs text-slate-500 mb-1">Platform</p>
              <p className="text-sm text-slate-200">{platform.icon} {platform.label}</p>
            </div>

            <div>
              <p className="text-xs text-slate-500 mb-1">Reporter</p>
              <p className="text-sm text-slate-200 font-medium">{ticket.reporter_name}</p>
              <p className="text-xs text-slate-500">{ticket.reporter_email}</p>
            </div>

            {/* Assign — ALL users can assign */}
            <div>
            ...
            {ticket.trading_market && (
              <div>
                ...
                <p className="text-sm text-slate-200">{ticket.trading_market}</p>
              </div>
            )}
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-1">
                  {ticket.tags.map((tag: string) => (
                    <span key={tag} className="badge bg-[#252836] border-[#2d3142] text-slate-400 text-[10px]">{tag}</span>
                  ))}
                </div>
              </div>
            )}
